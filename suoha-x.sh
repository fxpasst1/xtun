...
case "$(uname -m)" in
	...
	if [ ! -f "x-tunnel-linux" ]
	then
	curl -L https://raw.githubusercontent.com/fxpasst1/xtun/main/bin/x-tunnel-linux-amd64 -o x-tunnel-linux
	fi
	if [ ! -f "opera-linux" ]
	then
	...
	if [ ! -f "x-tunnel-linux" ]
	then
	curl -L https://raw.githubusercontent.com/fxpasst1/xtun/main/bin/x-tunnel-linux-386 -o x-tunnel-linux
	fi
	if [ ! -f "opera-linux" ]
	then
	...
	if [ ! -f "x-tunnel-linux" ]
	then
	curl -L https://raw.githubusercontent.com/fxpasst1/xtun/main/bin/x-tunnel-linux-arm64 -o x-tunnel-linux	
	fi
	if [ ! -f "opera-linux" ]
	then
...
echo 可以访问 http://$(curl -4 -s https://raw.githubusercontent.com/fxpasst1/xtun/main/bin/trace | grep ip= | cut -d= -f2):$metricsport/metrics 查找 userHostname
}
