...
apt update && apt install -y curl wget jq tar unzip sudo file || apk add curl wget jq tar unzip bash file
...
if [[ $(file /usr/local/bin/x-tunnel) == *"HTML"* || $(wc -c <"/usr/local/bin/x-tunnel") -lt 10000 ]]; then
    echo -e "${RED}错误：x-tunnel 下载失败 (下到了 HTML 网页)，请检查仓库路径。${PLAIN}"
    exit 1
fi
...
echo -e "------------------------------------------------------"
printf "%-25s %-15s\n" "nat-xray (Xray Core):" "$(systemctl is-active nat-xray)"
printf "%-25s %-15s\n" "nat-xtun (XTun Tunnel):" "$(systemctl is-active nat-xtun)"
printf "%-25s %-15s\n" "nat-cf   (CF Tunnel):"   "$(systemctl is-active nat-cf)"
echo -e "------------------------------------------------------"
...
echo -e " 查看端口占用:  ${GREEN}netstat -tlpn${PLAIN}"
echo -e "======================================================"
